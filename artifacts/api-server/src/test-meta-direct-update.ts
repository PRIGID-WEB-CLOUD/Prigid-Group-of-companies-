...
async function run() {
  ...
  const productToUpdate = d1.data[0];
  const graphId = productToUpdate.id;
  
  console.log(`Trying to update Graph ID: ${graphId}`);

  // Direct UPDATE using Graph ID via POST
  const updateData = new URLSearchParams();
  updateData.append("access_token", token);
  updateData.append("price", "999.99 EUR");

  const r2 = await fetch(`https://graph.facebook.com/v21.0/${graphId}`, { 
    method: "POST",
    body: updateData
  });
  const d2 = await r2.json();
  console.log("Direct UPDATE response:", d2);
