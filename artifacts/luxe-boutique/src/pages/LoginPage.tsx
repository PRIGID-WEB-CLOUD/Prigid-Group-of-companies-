...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (authError) {
      if (authError === "google_not_configured") {
        setError("Google authentication is not configured in settings.");
      } else if (authError === "facebook_not_configured") {
        setError("Facebook authentication is not configured in settings.");
      } else {
  ...
  const handleGoogleRedirect = () => {
    window.location.href = `/api/auth/google?returnUrl=${encodeURIComponent(window.location.origin + "/account")}`;
  };

  const handleFacebookRedirect = () => {
    window.location.href = `/api/auth/facebook?returnUrl=${encodeURIComponent(window.location.origin + "/account")}`;
  };
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md bg-white p-12 shadow-2xl rounded-2xl space-y-8">
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 border-2 border-slate-100 hover:border-slate-300 py-3.5 rounded-xl text-sm font-semibold text-slate-700 transition-all active:scale-[0.98]"
          >
            <svg width="18" height="18" viewBox="0 0 18 18" fill="currentColor" className="text-[#1877F2]">
              <path d="M18 9c0-4.97-4.03-9-9-9S0 4.03 0 9c0 4.49 3.28 8.21 7.59 8.89v-6.28H5.31V9h2.28V7.33c0-2.25 1.34-3.5 3.4-3.5.99 0 2.02.18 2.02.18v2.22h-1.14c-1.11 0-1.46.69-1.46 1.39V9h2.5l-.4 2.61h-2.1v6.28C14.72 17.21 18 13.49 18 9z"/>
            </svg>
